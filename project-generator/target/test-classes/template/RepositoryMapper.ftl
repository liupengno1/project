...
<mapper namespace="com.joinway.${appName}.${componentName}.dao.repository.${Table}Repository">
	<!--
		此文件初版由工具生成，仅需生成一次，请定制开发
		生成时间: ${now}
	-->
	<resultMap id="${Table}GridResultMap" type="com.joinway.${appName}.${componentName}.bean.domain.${Table}Grid" extends="com.joinway.${appName}.data.dao.mapper.${Table}Mapper.BaseResultMap">
	</resultMap>

	<sql id="Select">
		select
			<include refid="com.joinway.${appName}.data.dao.mapper.${Table}Mapper.Base_Column_List" />
		from ${t} where 1
...
<#if (textColumnCount == 0)>	
	<select id="select" parameterType="map" resultMap="com.joinway.${appName}.data.dao.mapper.${Table}Mapper.BaseResultMap">
<#else>
	<select id="select" parameterType="map" resultMap="com.joinway.${appName}.data.dao.mapper.${Table}Mapper.ResultMapWithBLOBs">
</#if>
