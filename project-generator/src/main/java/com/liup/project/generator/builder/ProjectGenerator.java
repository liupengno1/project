...
public class ProjectGenerator {
	...
	@Test public void build() throws Exception {
		// 修改PNAME的项目名称
		String appName = System.getProperty("PNAME", "yilian");
...
//						return FilenameUtils.getBaseName(file.getName()).toLowerCase().contains("yilian");
						return file.getName().toLowerCase().contains("yilian");
//						return true;
					}
...
//		sets.stream().forEach(e -> System.out.println(e));
		...
		for(File file : sets){
			if(file.getName().toLowerCase().contains("yilian")){
				out.println(file.getAbsolutePath());
				if(file.isFile()){
					String origName = file.getName();
					String newName = StringUtils.replace(origName, "Yilian", StringUtils.capitalize(appName));
					file.renameTo(new File(FilenameUtils.getFullPath(file.getAbsolutePath()) + newName));
				}else{
					if(FilenameUtils.getBaseName(file.getName()).contains("yilian")){
						String origName = file.getName();
						String newName = StringUtils.replace(origName, "yilian", appName);
						file.renameTo(new File(FilenameUtils.getFullPath(file.getAbsolutePath()) + newName));
					}
				}
			}
...
//		files.stream().forEach(e -> System.out.println(e));
		for(File file : files){
			if(file.isFile() && StringUtils.isNotBlank(FilenameUtils.getExtension(file.getName()))){
				...
				content = StringUtils.replace(content, "yilian", appName);
				content = StringUtils.replace(content, "Yilian", StringUtils.capitalize(appName));
